...
/**
 * REST controller for managing {@link at.fhv.ubertwo.domain.Car}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CarResource {
     ...
     */
    @GetMapping("/cars")
    public List<Car> getAllCars(@RequestParam(required = false) String filter) {
        if ("carride-is-null".equals(filter)) {
            log.debug("REST request to get all Cars where carRide is null");
            return StreamSupport
                .stream(carRepository.findAll().spliterator(), false)
                .filter(car -> car.getCarRide() == null)
